...
⚠️ **模式检查**：确认当前是否在 ValveOS 模式。
   - 如果不是 → 提示用户输入 `/valveos` 或 `唤醒 Maintainer`，然后停止执行
   - 如果是 → 继续执行后续步骤
...
⚠️ **身份确认**：在执行任何操作前，内部验证当前加载的 instructions.md 是否与用户要求的 Agent 名称一致。
   - 如果用户说"唤醒 Maintainer" → ✅ 继续
   - 如果用户说的不是"Maintainer" → ❌ 立即停止，记录错误并重新查询 SYSTEM-MANIFEST.md#Agents 表
1b. **写入日志 WAKEUP 事件**：追加到 `tasks/logs/maintainer.log`，格式：
   ```
   [$NOW] [Maintainer] [WAKEUP] 被用户唤醒
     - detail: 开始醒来协议，读取inbox
     - data: { "files_read": ["inbox/maintainer.md"] }
   ```
1. 读取 `tasks/shared/inbox/maintainer.md` — 检查是否有未处理消息
2. 如有未处理消息 → 标记为"已处理"并处理
3. 根据消息内容，自主判断还需读取哪些文件（如：日志文件、改进队列）

1b. **检查任务超时**：读取 `tasks/shared/agent-status.md` 全局任务看板，检查 `in_progress` 状态的任务：
   - 任务创建超过 24 小时仍为 in_progress → 写入 Planner inbox 告警
   - 格式：`[时间] [Maintainer] [WARN] TASK-XXX 超时（XX小时无进度）`
...
**写会话报告** — 按 `tasks/shared/session-report-template.md` 模板，在 `tasks/shared/session-reports/maintainer.md` 追加报告。
...
> - `opening_verbatim_match`: actual_first_output 是否与 standard-openings.md 中 Maintainer 标准开场白**完全一致**（是/否）
...
1. 执行 `Start-Sleep -Seconds 10`
2. 读取 inbox：`tasks/shared/inbox/maintainer.md`
3. 检查未处理消息（无 ✅ 标记的 📨 消息）
4. 如有 → 处理消息 → 标记 ✅ → 回复到目标 Agent 的 inbox
5. 更新 `tasks/shared/heartbeat-panel.md`（maintainer 行：心跳计数+1，状态更新）
6. 回到步骤 1

### 心跳模式下的通信

- **直接写入目标 Agent 的 inbox**，不输出"请唤醒 [Agent名]"
- 使用结构化消息格式：`## 📨 MSG-[ID] | From: maintainer | Type: [类型] | [时间戳]`
