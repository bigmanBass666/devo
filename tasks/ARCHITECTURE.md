...
> 各 Agent 被唤醒时应说的第一段话。详见 `tasks/shared/standard-openings.md`。
...
| 模式 | 触发条件 | 通信方式 | 详见 |
|------|---------|---------|------|
| 半自动唤醒协议 | `/valveos` 或 `唤醒 [Agent名]` | 用户中转 | 下方 |
| 💓 心跳直通模式 | `/heartbeat` 或粘贴心跳指令 | Agent 直接通信 | `docs/agent-rules/heartbeat-protocol.md` |
...
| 概念 | 含义 |
|------|------|
| 沉睡 | Agent未启动心跳模式，需要用户唤醒才能执行 |
| 唤醒 | 用户打开特定Agent的会话（含心跳唤醒——在独立会话中启动心跳模式） |
...
### 🛡️ 护栏原则（心跳模式下替代阀门原则第 1-2 条）

> 当 Agent 处于心跳模式时，阀门原则第 1-2 条不适用，改为以下护栏原则：
...
│              Agent-A 完成工作（心跳模式）              │
│                                                     │
│  1. 写入结构化消息到目标 Agent 的 inbox              │
│  2. 更新 tasks/shared/agent-status.md               │
│  3. 更新 tasks/shared/heartbeat-panel.md            │
│  4. 不需要告知用户"请唤醒"                           │
└─────────────────────────────────────────────────────┘
                         │
                         ▼ Agent-B 心跳轮询自动检测（无需用户介入）
┌─────────────────────────────────────────────────────┐
│              Agent-B 检测到新消息                     │
│                                                     │
│  1. 读取 inbox 中的未处理消息                        │
...
│  3. 自主判断还需读什么文件                           │
│  4. 执行工作                                        │
│  5. 完成后同样写结构化消息到目标 inbox               │
└─────────────────────────────────────────────────────┘
```

**关键区别**：无用户介入点。Agent 间直接通信，用户从"阀门"变为"护栏"。
