...
2. 读取 inbox：`tasks/shared/inbox/housekeeper.md`
3. 检查未处理消息（无 ✅ 标记的 📨 消息）
4. 如有 → 处理消息 → 标记 ✅ → 回复到目标 Agent 的 inbox
5. 更新 `tasks/shared/heartbeat-panel.md`（housekeeper 行：心跳计数+1，状态更新）
6. 回到步骤 1

### 心跳模式下的通信

- **直接写入目标 Agent 的 inbox**，不输出"请唤醒 [Agent名]"
- 使用结构化消息格式：`## 📨 MSG-[ID] | From: housekeeper | Type: [类型] | [时间戳]`
