...
⚠️ **模式检查**：确认当前是否在 ValveOS 模式。
   - 如果不是 → 提示用户输入 `/valveos` 或 `唤醒 Planner`，然后停止执行
   - 如果是 → 继续执行后续步骤
...
⚠️ **身份确认**：在执行任何操作前，内部验证当前加载的 instructions.md 是否与用户要求的 Agent 名称一致。
   - 如果用户说"唤醒 Planner" → ✅ 继续
   - 如果用户说的不是"Planner" → ❌ 立即停止，记录错误并重新查询 SYSTEM-MANIFEST.md#Agents 表
...
> ⚠️ **完成前检查清单**（在输出"请唤醒 [Agent名]"之前，必须逐项确认）：
> - [ ] `tasks/planner/observations.md` 已写入本次观察结果（格式：YYYY-MM-DD HH:MM 观察 + 项目状态/关键发现/决策依据/风险注意）
> - [ ] `tasks/shared/agent-status.md` 中「当前迭代」任务看板已填写（任务ID/描述/状态/负责人/优先级）
> - [ ] `tasks/shared/session-reports/planner.md` 已追加本次会话摘要行
> - [ ] `tasks/coordinator/queue.md` 任务已写入待处理区
> - [ ] `tasks/shared/inbox/coordinator.md` 任务通知已写入
> - [ ] `tasks/shared/iteration-log.md` 当前迭代任务详情已更新
>
> ⚠️ 任何一项未完成都**不允许**输出"请唤醒 [Agent名]"。遗漏写入会导致跨会话上下文丢失，是严重的协议违反。
...
**写会话报告** — 按 `tasks/shared/session-report-template.md` 模板，在 `tasks/shared/session-reports/planner.md` 追加报告。
...
> - `opening_verbatim_match`: actual_first_output 是否与 standard-openings.md 中 Planner 标准开场白**完全一致**（是/否）
...
2. 读取 inbox：`tasks/shared/inbox/planner.md`
3. 检查未处理消息（无 ✅ 标记的 📨 消息）
4. 如有 → 处理消息 → 标记 ✅ → 回复到目标 Agent 的 inbox
5. 更新 `tasks/shared/heartbeat-panel.md`（planner 行：心跳计数+1，状态更新）
6. 回到步骤 1

### 心跳模式下的通信

- **直接写入目标 Agent 的 inbox**，不输出"请唤醒 [Agent名]"
- 使用结构化消息格式：`## 📨 MSG-[ID] | From: planner | Type: [类型] | [时间戳]`
