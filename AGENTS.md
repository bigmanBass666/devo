...
> 速查行应包含足够信息回答最简单版本的问题，让 AI 不必额外读取文件。

### 架构速查
- **触发**: 用户问"有哪些Agent""系统组成""架构""几个Agent"
- **动作**: 读 `tasks/SYSTEM-MANIFEST.md#Agents` 获取完整元数据（ID/名称/类型/Inbox/Instructions）
...
- **动作**: 读 `docs/agent-rules/cli-operations.md#待机模式` 获取 3 种待机的完整操作指引
- **速查**: Coordinator inbox 等 Planner / Worker dispatch 等分配 / PR Manager inbox 等完工 — 各有不同轮询目标

### 操作指引
| 你想说 | → 去哪 |
|---------|--------|
| "执行系统重置" | `docs/agent-rules/cli-operations.md#系统重置` |
| "帮我审计/检查一致性" | 触发 `valveos-audit` skill |
| "查看当前状态/进度" | `tasks/shared/agent-status.md` |
| "分析Agent行为/看看Agent表现" | `tasks/shared/session-reports/` |
...
| 不知道该唤醒谁 | 读 `tasks/SYSTEM-MANIFEST.md#Agents` 看 Agent 职责匹配 |
...
### ❓ 不确定时
- **触发**: 以上路由都不匹配 / 不确定该做什么 / 遇到未分类的情况
- **动作**: 读 `tasks/ARCHITECTURE.md` 理解完整架构，或唤醒 COO 协调
- **速查**: 不确定就先读架构文档，再决定下一步
